...
(doom! :completion (corfu +orderless) (vertico +icons)
       :ui         doom (modeline -light) nav-flash ophints
                   neotree unicode (popup +all +defaults)
                   (vc-gutter +pretty) vi-tilde-fringe
                   (window-select +numbers +switch-window)
       :editor     (evil +everywhere) file-templates fold
                   format snippets (whitespace +guess +trim) word-wrap
       :emacs      (dired +icons) electric (ibuffer +icons) tramp undo vc
       :term       vterm
       :checkers   syntax
       :tools      (ansible +lsp) (debugger +lsp) (docker +lsp)
                   editorconfig (eval +overlay)
                   lookup (lsp -peek) magit make
                   taskrunner tree-sitter
       :os         tty
       :lang       (cc +lsp) (dart +flutter) data emacs-lisp
                   (go +lsp) (java +lsp)
                   (javascript +lsp +tree-sitter) (json -lsp)
                   (lua +lsp) markdown (org +hugo +dragndrop)
                   (python +lsp) rest
