...
(defun uki/enable-eldoc-for-lsp-h ()
    ...
    (interactive)
    (require 'eldoc nil t)
    (add-hook! 'eldoc-documentation-functions :local #'lsp-eldoc-function)
    (eldoc-mode 1))
#+end_src
** Desktop-save
#+begin_src emacs-lisp
...
;;(defun uki/org-hugo-get-point-of-export-header (&optional property)
;;  "Returns point value at org headline conteining property EXPORT_FILE_NAME.
;;Will go up the hierarchy and look at sibiling headlines when searching
;;for the main one."
;;  (unless property
...
;;      (outline-up-heading nil t))
;;    (while (or (not (org-entry-get (point) property nil))
;;               (and t))
;;      (outline-up-heading nil t))
;;    (point)))

(defun uki/org-hugo-set-time-property ()
  "Set or refresh tree's timestamp."
  (interactive)
  (save-excursion
  (let ((prop (if (org-entry-get (point) "EXPORT_DATE" nil)
                     "EXPORT_HUGO_LASTMOD"
                   "EXPORT_DATE")))
    (org-set-property prop (format-time-string org-hugo-date-format)))))

(defun uki/org-hugo-set-properties ()
  "Sets properties required by Hugo."
  (interactive)
  (save-excursion
    (let* ((default (or (org-entry-get (point) "EXPORT_FILE_NAME" nil)
    ...
    (uki/org-hugo-set-time-property)))
